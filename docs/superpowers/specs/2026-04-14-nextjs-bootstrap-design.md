...
- BottomTabBar が指す 5 ルートの placeholder page (`/home`, `/plan`, `/chat`, `/progress`, `/profile`)
...
- Chat / AI の実機能 (SSE streaming, AI SDK adapter, 経路 A / C) → Plan 08+
...
- Cookie は BFF が Cognito token をそのまま HttpOnly 属性付きで保持する
- `SESSION_SECRET` のような独自署名用 secret は不要。独自セッション JWT や暗号化 wrapper cookie は作らない
...
`/api/*`, `/_next/*`, `favicon.ico` はマッチャーで除外。
...
実装では Tailwind CSS v4 の `@theme` または CSS custom properties を用いて、`bg-canvas`, `text-primary-500`, `text-neutral-500` のような utility が使える状態にする。
...
Plan 05 の実装済み Lambda は現在以下の payload を返す:

```typescript
// GET /api/proxy/users/me/profile
// -> 200 { profile: Record<string, unknown> }
//
// PATCH /api/proxy/users/me/profile
// -> 200 { profile: Record<string, unknown> }
```

- `fetch-user-profile` は `ok({ profile: stripKeys(Item) })` を返す
- `update-user-profile` も `ok({ profile: stripKeys(Attributes) })` を返す
- このため frontend hook は `response.profile` を unwrap して使う
- `UpdateUserProfileInputSchema` は PATCH 入力検証専用。GET レスポンス全体の検証には使わない

### useProfile hook

```typescript
// GET /api/proxy/users/me/profile → TanStack useQuery
//   現在の runtime payload は { profile: ... } なので response.profile を unwrap
